...
一方で，使用するクラスタ計算機システムの環境が下記のような場合，  
- シェル bash
- gcc 4.3.4  
MPI環境:  
  - OpenMPI 1.5.3  
...
GCCが推奨環境に比べ古すぎるのもあるが，C++11以降の規格が使用できると便利なこともあり，開発環境として以下の環境をユーザのローカルフォルダにインストールした．
- gcc 6.3.0
- OpenMPI 2.1.2
- FFTW 3.3.7
...
```
makeファイルを作成する．  
下記は見やすいように改行しているが， `./gcc-6.3.0/configure` 以降の __すべてのオプションを改行せずに続けて入力すること．__  
...
(ここでは `$HOME/local/openmpi-2.1.2` にインストールする)
```bash
$ ../openmpi-2.1.2/configure --prefix=$HOME/local/openmpi-2.1.2 CC=gcc CXX=g++ F77=gfortran FC=gfortran
```
コンパイルおよびインストール
```bash
$ make
...
あるいは[おまけ2](#複数のgccバージョンの切り替え)のスクリプトを利用する．
```bash
target=$HOME"/local/openmpi-2.1.2"      # prefixで指定したOpenMPIのフォルダ
$ export PATH=${target}/bin:$PATH
$ export LD_LIBRARY_PATH=${target}/lib:$LD_LIBRARY_PATH
```
...
GCCの `configure` と同様 __すべてのオプションを改行せずに続けて入力すること．__  
(ここでは `$HOME/local/fftw-3.3.7` にインストールする)  
```bash
$ ../fftw-3.3.7/configure --prefix=$HOME/local/fftw-3.3.7  
...
```bash
-I${HOME}/local/fftw-3.3.7/include
-L${HOME}/local/fftw-3.3.7/lib -lfftw3f_mpi -lfftw3f -lm
```
をコンパイルオプションに追加する．  
ここで，`include` フォルダと `lib` フォルダは path を明示的に指定する必要があるが，コンパイル時の `--prefix=***` に指定したディレクトリ以下の構成は通常決まっているので，下記のように環境変数を追加しておくと便利である．
```bash
export FFTW_ROOT=$HOME/local/fftw-3.3.7
```
この環境変数 `FFTW_ROOT` を利用する場合，必要なコンパイルオプションは下記のようになる．
```bash
-I${FFTW_ROOT}/include
-L${FFTW_ROOT}/lib -lfftw3f_mpi -lfftw3f -lm
```

FDPSのParticleMesh機能拡張は単精度版のfftwを使用するが，ほかの精度のバージョンも使いたければ
configure 設定の最後につけた `--enable-float --enable-sse` を下表のように書き換える．

| オプション | 効果 |  
|:-----------|:-----|
| --enable-float --enable-sse | 標準の単精度(32bit)浮動小数点 (上記の例) |
...
あるいは[おまけ2](#複数のgccバージョンの切り替え)のスクリプトを利用する．
```bash
target=$HOME"/local/fftw-3.3.7"      # prefixで指定したFFTwのフォルダ名
$ export LD_LIBRARY_PATH=${target}/lib:$LD_LIBRARY_PATH
```
...
このドキュメントに従って環境を構築しており，環境変数 `FFTW_ROOT` が定義されているなら，下記のようになる．
...
ただし，OpenMPI はコンパイルに用いた GCC と，FFTw はコンパイルに用いた OpenMPI および GCC と同時に使用する必要がある点に注意すること．
