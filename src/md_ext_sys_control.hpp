...
namespace EXT_SYS {
    ...
    private:
        ...
        std::vector<PS::F64> w_coef;
        std::vector<PS::F64> x_nhc;
        std::vector<PS::F64> v_nhc;
    ...
    public:
        ...
        void write_resume() const;
        void load_resume();

        void broadcast(const PS::S32 root = 0){
            COMM_TOOL::broadcast(this->init_flag, root);
            COMM_TOOL::broadcast(this->n_chain,   root);
            COMM_TOOL::broadcast(this->n_rep,     root);
            COMM_TOOL::broadcast(this->n_nys,     root);
            COMM_TOOL::broadcast(this->NVT_freq,  root);
            COMM_TOOL::broadcast(this->NPT_freq,  root);
            COMM_TOOL::broadcast(this->v_press,   root);
            COMM_TOOL::broadcast(this->w_coef,    root);
            COMM_TOOL::broadcast(this->x_nhc,     root);
            COMM_TOOL::broadcast(this->v_nhc,     root);
        }
        ...
        void print() const {
            ...
            if(this->init_flag){
                oss << "  n_chain  = " << this->n_chain  << "\n"
                    << "  n_rep    = " << this->n_rep    << "\n"
                    << "  n_nys    = " << this->n_nys    << "\n"
                    << "  NVT_freq = " << std::scientific << this->NVT_freq << "  (normalized)" << "\n"
                    << "  NPT_freq = " << std::scientific << this->NPT_freq << "  (normalized)" << "\n";
            } else {
                oss << "  --- not initialized ---\n";
            }
            oss << "\n";
    ...
    void Controller::init(const PS::S32 &n_chain,
        ...
        this->n_rep    = n_rep;
        this->n_nys    = n_nys;
        this->NVT_freq = NVT_freq*2.0*Unit::pi*Unit::norm_time;
        this->NPT_freq = NPT_freq*2.0*Unit::pi*Unit::norm_time;

        switch (n_nys) {
            case 3:
                this->w_coef.clear();
                this->w_coef.push_back( 1.0/(2.0 - std::pow(2.0, 1.0/3.0)) );
                this->w_coef.push_back( 1.0 - 2.0*this->w_coef.at(0) );
                this->w_coef.push_back( this->w_coef.at(0) );
            break;

            case 5:
                this->w_coef.clear();
                this->w_coef.push_back( 1.0/(4.0 - std::pow(4.0, 1.0/3.0)) );
                this->w_coef.push_back( this->w_coef.at(0) );
                this->w_coef.push_back( 1.0 - 4.0*this->w_coef.at(0) );
                this->w_coef.push_back( this->w_coef.at(0) );
                this->w_coef.push_back( this->w_coef.at(0) );
            break;
        ...
        this->v_nhc.resize(n_chain);
        this->x_nhc.resize(n_chain);
        std::fill(this->v_nhc.begin(), this->v_nhc.end(), 0.0);
        std::fill(this->x_nhc.begin(), this->x_nhc.end(), 0.0);
    ...
    void Controller::kick(const PS::F64 &dt,
        ...
        //--- kick atom
        PS::S64 n = psys.getNumberOfParticleLocal();
    ...
    void Controller::write_resume() const { throw std::logic_error("under developing"); }
    void Controller::load_resume()        { throw std::logic_error("under developing"); }
    ...
    void Controller::nose_hoover_chain(const PS::S64 &n_rigid_local,
        ...
        //--- set local constant
        std::vector<PS::F64> g_nhc;
        std::vector<PS::F64> q_mass;
        std::vector<PS::F64> w_dti;

        g_nhc.resize(this->n_chain);
        q_mass.resize(this->n_chain);
        w_dti.resize(this->n_nys);
        ...
        for(PS::S32 i_rep=0; i_rep<this->n_rep; ++i_rep){
            for(PS::S32 i_nys=0; i_nys<this->n_nys; ++i_nys){
                //--- update thermostat velocities (heat source -> atom)
                this->nhc_update_vel_forward(g_nhc, w_dti.at(i_nys));

                //--- update particle velocity
                PS::F64 factor = std::exp(-w_dti.at(i_nys)*this->v_nhc.at(0));
                scale          = scale*factor;
                eng_kinetic    = eng_kinetic*(factor*factor);

                //--- update thermostat position
                for(PS::S32 i_chain=0; i_chain<this->n_chain; ++i_chain){
                    this->x_nhc.at(i_chain) += this->v_nhc.at(i_chain)*w_dti.at(i_nys);
                }
    ...
    void Controller::nose_hoover_chain_andersen(const PS::S64 &n_rigid_local,
        ...
        PS::F64 v_mass         = PS::F64(n_deg_free + 3)*norm_tgt_temp/std::pow(this->NPT_freq, 2);
        ...
        PS::F64 scale   = 1.0;
        g_nhc.at(0)     = (eng_kinetic + v_mass*std::pow(this->v_press, 2) - g_n1kt)/q_mass.at(0);
        ...
        for(PS::S32 i_rep=0; i_rep<this->n_rep; ++i_rep){
            for(PS::S32 i_nys=0; i_nys<this->n_nys; ++i_nys){
                ...
                //--- update particle velocity
                PS::F64 factor = std::exp( -w_dti.at(i_nys)*(this->v_nhc.at(0) + n_deg_free_inv*this->v_press) );
                scale          = scale*factor;
                eng_kinetic    = eng_kinetic*(factor*factor);
        ...
        //--- energy of thermostat
        PS::F64 eng_nhc  = this->nhc_calc_eng_thermostat(n_deg_free, norm_tgt_temp, q_mass);
                eng_nhc += norm_tgt_temp*this->x_nhc.at(0);
                eng_nhc += 0.5*v_mass*std::pow(this->v_press, 2) + norm_tgt_press*Normalize::getVol();
    ...
    void Controller::nhc_calc_q_mass(const PS::S64 &n_deg_free,
        ...
        PS::F64 NVT_freq_sqinv = 1.0/std::pow(this->NVT_freq, 2);
        g_nkt                  = PS::F64(n_deg_free)*norm_tgt_temp;

        q_mass.at(0) = g_nkt*NVT_freq_sqinv;
        for(PS::S32 i=1; i<this->n_chain; ++i){
    ...
    void Controller::nhc_calc_w_dti(const PS::F64 &dt,
        ...
        for(PS::S32 i=0; i<this->n_nys; ++i){
            w_dti.at(i) = 0.5*this->w_coef.at(i)*dt/PS::F64(this->n_rep);
        }
    }
    ...
    void Controller::nhc_update_vel_forward(const Tarray  &g_nhc,
        ...
        this->v_nhc.back() += g_nhc.back()*0.5*w_dt;

        for(PS::S32 next=this->n_chain-2; next>=0; --next){
            PS::S32 previous = next + 1;
            PS::F64 factor   = std::exp(-0.25*w_dt*this->v_nhc.at(previous));
            this->v_nhc.at(next) = ( this->v_nhc.at(next)*factor
    ...
    void Controller::nhc_update_vel_backward(      Tarray  &g_nhc,
        ...
        for(PS::S32 i_chain=0; i_chain<this->n_chain-1; ++i_chain){
            PS::F64 factor          = std::exp(-0.25*w_dt*this->v_nhc.at(i_chain+1));
            this->v_nhc.at(i_chain) = ( this->v_nhc.at(i_chain)*factor
                                        +     g_nhc.at(i_chain)*0.5*w_dt )*factor;
            g_nhc.at(i_chain+1)     = ( q_mass.at(i_chain)*std::pow(this->v_nhc.at(i_chain), 2)
                                        - norm_tgt_temp )
                                    /q_mass.at(i_chain+1);
        }
    ...
    inline void Controller::andersen_update_v_press(const PS::F64 &w_dt,
                                                    const PS::F64 &g_press){
        PS::F64 factor = std::exp(-0.25*w_dt*this->v_nhc.at(0));
        this->v_press  = (this->v_press*factor + 0.5*w_dt*g_press)*factor;
    }
    ...
    PS::F64 Controller::nhc_calc_eng_thermostat(const PS::S64 &n_deg_free,
        ...
        PS::F64 eng = PS::F64(n_deg_free)*norm_tgt_temp*this->x_nhc.at(0);
        for(PS::S32 i=0; i<this->n_chain; ++i){
            eng += 0.5*q_mass.at(i)*std::pow(this->v_nhc.at(i), 2);
            eng += norm_tgt_temp*this->x_nhc.at(i);
        }
        return eng;
    }
