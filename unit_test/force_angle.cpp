...
void test_init(Tpsys &psys){
    ...
    psys[2].setPos( PS::F64vec(0.5) + Initialize::rot_y( Normalize::normPos( PS::F64vec(0.995, 0.0, 0.0) ), Unit::pi*112.5/180.0 ) );
    ...
    for(PS::S64 i=0; i<psys.getNumberOfParticleLocal(); ++i){
        ...
        if(psys[i].getAtomID() == 2){
            ...
            PS::F64vec pos_local = pos_tmp - Normalize::realPos( PS::F64vec(0.5) );
                       pos_local = Initialize::rot_y(pos_local, 2.0*Unit::pi/PS::F64(n_loop));
