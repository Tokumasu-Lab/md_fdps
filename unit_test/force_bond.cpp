...
void test_move(Tpsys &psys){
    ...
    for(PS::S64 i=0; i<psys.getNumberOfParticleLocal(); ++i){
        ...
        if(psys[i].getAtomID() == 1){
            ...
            pos_tmp.x += 3.4/PS::F64(n_loop);         //  setting test distance range [angstrom]
        }
...
//    std::cout << "  data_proc = " << data_proc << std::endl;
    ...
    //--- for Intra force
    oss << std::setw(15) << std::setprecision(7) << buf.getPotBond();
...
int main(int argc, char* argv[]) {
        ...
        //--- data logger
        Observer::Energy      eng;
                              eng.file_init("test_force_bond_eng.dat");
        Observer::FilePrinter printer;
                              printer.file_init("test_force_bond_log.dat", 0);
