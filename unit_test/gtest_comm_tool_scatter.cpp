...
//     module location: ./generic_ext/comm_tool_scatter.hpp
//=======================================================================================
...
TEST_F(ScatterBasic, VecInt){
    const int n_proc = PS::Comm::getNumberOfProc();
    const int rank   = PS::Comm::getRank();

    for(int i=0; i<n_proc; ++i){
        auto recv_vi = COMM_TOOL::scatter(data[i].vec_vi, i);

        static_assert(std::is_same<decltype(recv_vi), std::vector<int> >::value == true, "auto type check");

        EXPECT_EQ(recv_vi, data.at(i).vec_vi.at(rank) ) << "from = " << i << ", to = " << rank;
    }
}

TEST_F(ScatterBasic, VecVecInt){
    const int n_proc = PS::Comm::getNumberOfProc();
    const int rank   = PS::Comm::getRank();

    for(int i=0; i<n_proc; ++i){
        auto recv_vvi = COMM_TOOL::scatter(data[i].vec_vvi, i);

        static_assert(std::is_same<decltype(recv_vvi), std::vector<std::vector<int>> >::value == true, "auto type check");

        EXPECT_EQ(recv_vvi, data.at(i).vec_vvi.at(rank) ) << "from = " << i << ", to = " << rank;
    }
}

TEST_F(ScatterBasic, Str){
    const int n_proc = PS::Comm::getNumberOfProc();
    const int rank   = PS::Comm::getRank();

    for(int i=0; i<n_proc; ++i){
        auto recv_str = COMM_TOOL::scatter(data[i].vec_s, i);

        static_assert(std::is_same<decltype(recv_str), std::string >::value == true, "auto type check");

        EXPECT_EQ(recv_str, data.at(i).vec_s.at(rank) ) << "from = " << i << ", to = " << rank;
    }
}

TEST_F(ScatterBasic, VecStr){
    const int n_proc = PS::Comm::getNumberOfProc();
    const int rank   = PS::Comm::getRank();

    for(int i=0; i<n_proc; ++i){
        auto recv_vs = COMM_TOOL::scatter(data[i].vec_vs, i);

        static_assert(std::is_same<decltype(recv_vs), std::vector<std::string> >::value == true, "auto type check");

        EXPECT_EQ(recv_vs, data.at(i).vec_vs.at(rank) ) << "from = " << i << ", to = " << rank;
    }
}

TEST_F(ScatterBasic, VecPairIntFloat){
    const int n_proc = PS::Comm::getNumberOfProc();
    const int rank   = PS::Comm::getRank();

    for(int i=0; i<n_proc; ++i){
        auto recv_vp_if = COMM_TOOL::scatter(data[i].vec_vp_if, i);

        static_assert(std::is_same<decltype(recv_vp_if), std::vector<std::pair<int,
                                                                               float>> >::value == true, "auto type check");

        EXPECT_EQ(recv_vp_if, data.at(i).vec_vp_if.at(rank) ) << "from = " << i << ", to = " << rank;
    }
}
